#===============================================================================
# Python script for shared features detection.
#===============================================================================
import json
import sys
import pymongo
...
DURATION_THRESHOLD = 1  # Filtering the number of seconds for shared data points
COUNTER_VALUE = .25  # How much time does each 'sample' represent
...
def parse_raw_data(session_key, user, key, type):
    ...
    document = source_collection.find_one(
    ...
    for user in users_final:
        for user2 in users_final:
            ...
            if (user != user2) and (key1 not in paired_detections) and (key2 not in paired_detections):
                output = compute_pair_shared(
                    raw_data[user], raw_data[user2], length)
...
if __name__ == "__main__":
    ...
    collection = database['[RESULT DATABSE]']
